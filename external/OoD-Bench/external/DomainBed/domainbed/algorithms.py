...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, teacher_arch, hparams):
        super(ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer(input_shape, num_classes, teacher_arch, self.hparams)
    ...
    def forward(self, x, is_feat=True, preact=False):
        # print('x ', x.isnan().any(), '--')
        # print('max ', torch.max(x), '--')
        # print('min ', torch.min(x), '--')
        feat_t, pred = self.featurizer(x, is_feat, preact)
        # print('1==>', pred.isnan().any(), '==>')
        pred = self.classifier(pred)
        # print('2==>', pred.isnan().any(), '==>')
        pred2 = self.predict(x)
        # print('3==>', pred2.isnan().any(), '==>')
        return feat_t, pred2
