...
sys.path.append('external/OoD-Bench/external/DomainBed/')
...
if __name__ == "__main__":
    ...
    hparams['teacher_arch'] = args.teacher_arch
    ...
    algorithm = algorithm_class(dataset.input_shape, dataset.num_classes,
                                len(dataset) - len(args.test_envs) - len(args.val_envs), 
                                args.teacher_arch, hparams)

    if algorithm_dict is not None:
        algorithm.load_state_dict(algorithm_dict)

    print(f'sending algorithm to {device}')
    algorithm.to(device)

    # data = torch.randn(32, 3, 224, 224)
    # data = data.to(device)
    # algorithm.eval()
    # feat_t, pred = algorithm(data, is_feat=True)
    # pred2 = algorithm.predict(data)

    print('zipping loaders...')
    ...
    def save_checkpoint(filename):
        ...
        save_dict = {
            ...
            "model_teacher_arch": args.teacher_arch
        }
    ...
    # save_checkpoint('model_last.pkl')
